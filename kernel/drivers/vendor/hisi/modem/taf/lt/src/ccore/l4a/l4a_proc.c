...
VOS_VOID l4a_help(VOS_VOID)
{
    ...
    {
        vos_printf("%25s: req msgid 0x%08x, cnf msgid 0x%08x\n", 
...
VOS_VOID l4a_DelCmdNode(L4A_RX_INFO_STRU* pstCmdNode)
{
    ...
    /*释放内存*/
    VOS_MemFree(MSP_L4_L4A_PID,pstCmdNode->pData);

    /*释放内存*/
    VOS_MemFree(MSP_L4_L4A_PID,pstCmdNode);
...
VOS_UINT32 l4a_AddCmdToListEx(L4A_DATA_MSG_STRU * pstReq, VOS_UINT32 ulATReqMsgId, VOS_UINT32 ulPSCnfMsgId)
{
    ...
    VOS_MemSet(pNewNode, 0, sizeof(L4A_RX_INFO_STRU));
    
    /*再申请一个请求缓存区*/
    pNewNode->pData = VOS_MemAlloc(MSP_L4_L4A_PID, DYNAMIC_MEM_PT, ulLen);
    if (NULL == pNewNode->pData)
    {
        VOS_MemFree(MSP_L4_L4A_PID, pNewNode);
        return ERR_MSP_FAILURE;
    }
    
    VOS_MemCpy(pNewNode->pData, pstReq, ulLen);
    ...
    l4a_StartTimer(&(pNewNode->fwCtrlTimer), pNewNode, L4A_CMD_TIMEOUT_TIME_LEN);
...
VOS_UINT32 l4a_AddCmdToList(L4A_DATA_MSG_STRU * pstReq, L4A_FW_MSG_FUN_TABLE_STRU* pstFunTable)
{
    ...
    l4a_AddCmdToListEx(pstReq, pstFunTable->ulAtReqMsgId, pstFunTable->ulPsCnfMsgId);
...
VOS_UINT32 l4aPacket2Ps(VOS_UINT32 ulRecverId, VOS_VOID *pData, VOS_UINT32 ulLen)
{
    ...
    VOS_MemCpy(pstAppToPsMsg->aucValue, pData,ulLen);
...
VOS_UINT32 l4a_DealPsIndCnfEntry(L4A_PS_MSG_STRU* pData)
{
    ...
    blist_for_each(me, pHeadNode)
    {
        ...
        l4a_StopTimer(&(pstNode->fwCtrlTimer));
        ...
        if(pfnCnfFun)
        {
            ...
            {
                l4a_StartTimer(&(pstNode->fwCtrlTimer), pstNode, L4A_CMD_TIMEOUT_TIME_LEN);
            }
        }
        else
        {
...
VOS_UINT32 l4a_TaskInit(VOS_VOID)
{
    ...
    VOS_MemSet(&g_stmntn, 0, sizeof(g_stmntn));
...
VOS_VOID l4a_MsgProc(MsgBlock* pMsgBlock)
{
    ...
    switch(pMsgBlock->ulSenderPid)
    {
        ...
        case DOPRA_PID_TIMER:
            ...
            l4a_DealTimeOutCmdEntry((L4A_RX_INFO_STRU*)(pTimer->ulPara));
            break;
...
VOS_UINT32 L4A_MailboxMsgProc(VOS_VOID)
{
    ...
    if(ulRet != BSP_OK)
    {
        VOS_MemFree(MSP_L4_L4A_PID, pMailBoxMsg);
                            ...
                            VOS_UINT32 ulPara3, VOS_UINT32 ulPara4)
{
    ...
    if(BSP_OK != BSP_MailBox_ComNotifyReg(EN_MAILBOX_SERVICE_LTE_COEX, l4a_MailboxCb))
    {
